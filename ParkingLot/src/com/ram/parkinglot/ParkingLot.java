...
public class ParkingLot{
    ...
    public boolean parkVehicle(Vehicle vehicle){
        ...
        for (int i = 0; i < levels.length; i++){
            if (levels[i].parkVehicle(vehicle)) {
                System.out.println(" Level " + i + " with Vehicle Number " + vehicle.licensePlate);
    ...
    public boolean leave(Vehicle vehicle,int lvl){
        ...
        System.out.println("Spot freed from  Level " + lvl + " and exited  " + vehicle.licensePlate + " of " + vehicle.Company);
    ...
    public boolean leave1(Vehicle vehicle,int lvl){
        System.out.println(" ------------------------------------------ ");
        levels[lvl].spotFreed();
        System.out.println("Spot freed from  Level " + lvl + " and exited  " + vehicle.licensePlate);
        ArrayList<String> VehicleList = this.CompanytoVehicle.get(vehicle.Company);
        if (VehicleList.contains(vehicle.licensePlate)) {
            VehicleList.remove(vehicle.licensePlate);
